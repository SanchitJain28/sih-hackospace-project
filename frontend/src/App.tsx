...
const App: React.FC = () => {
  ...
  const [debrisData, setDebrisData] = useState<Debris[]>(
    debrisJson as Debris[]
  );
  ...
  const fetchRealDebris = async () => {
    ...
    try {
      const response = await axios.get(
        "https://sih-hackospace-project-nylg.onrender.com/fetch-debris-positions"
      );
      ...
      console.log(
        `Transformed to ${transformedData.length} valid debris items`
      );
      ...
      setError(
        error instanceof Error ? error.message : "Failed to fetch debris data"
      );
    ...
    <div className="h-screen bg-black text-white overflow-hidden flex flex-col">
      ...
      <div className="flex flex-1 h-[calc(100vh-80px)]">
        ...
        <div className="flex-1 relative">
          <Globe debrisData={debrisData} filters={filters} />
        </div>

        <div className="w-96">
          <Alerts debrisData={debrisData} filters={filters} />
        </div>
      </div>
    </div>
  );

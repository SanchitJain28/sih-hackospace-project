...
const Alerts: React.FC<AlertsProps> = ({ debrisData, filters }) => {
  ...
  const filteredDebris: Debris[] = filterDebris(debrisData);
  ...
  const criticalAlerts: Debris[] = filteredDebris
  ...
  const highRiskPercentage = filteredDebris.length > 0 
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center space-x-3">
                ...
                <div>
                  ...
                  <p className="text-red-100 text-sm">Low altitude + High velocity</p>
                </div>
              </div>
        ...
        <div>
          ...
          <div className="space-y-3">
            {criticalAlerts.map(debris => (
              <div key={debris.id} className="bg-slate-800 border border-red-500 rounded-lg p-4">
                ...
                <div className="grid grid-cols-2 gap-2 text-sm text-gray-300">
                  <div>Altitude: <span className="text-white">{debris.altitude} km</span></div>
                  <div>Velocity: <span className="text-white">{debris.velocity} km/s</span></div>
                  <div>Size: <span className="text-white capitalize">{debris.size}</span></div>
                  <div>Mass: <span className="text-white">{debris.mass} kg</span></div>
                </div>
                
                <div className="mt-3 p-2 bg-red-900 bg-opacity-50 rounded border-l-4 border-red-500">
                  <p className="text-red-100 text-sm">
                    ⚠️ Collision risk due to low altitude and high velocity orbital parameters
                  </p>
                </div>
              </div>
            ))}
      ...
      <div className="bg-slate-800 rounded-lg p-4 border border-slate-700">
        ...
        <div className="space-y-2 text-sm">
          <div className="flex justify-between">
            ...
            <span className="text-white font-semibold">{filteredDebris.length}</span>
          </div>
          <div className="flex justify-between">
